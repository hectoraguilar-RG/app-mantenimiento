...
export default function AppMantenimiento() {
  const [perfiles, setPerfiles] = useState<Perfil[]>([]);
  const [usuarioActual, setUsuarioActual] = useState<Perfil | null>(null);
  ...
  const [modalAvance, setModalAvance] = useState<Tarea | null>(null);
  const [porcentajeAvance, setPorcentajeAvance] = useState(50);
  const [notaAvance, setNotaAvance] = useState('');
  const [fotoAvance, setFotoAvance] = useState<string | null>(null);
  ...
  function procesarFoto(e: React.ChangeEvent<HTMLInputElement>, callback: (url: string) => void) {
  ...
  async function crearTareaProgramada(e: React.FormEvent) {
    e.preventDefault();
  ...
  function alternarTecnicoSeleccionado(id: string) {
    setNuevaTarea(prev => {
